...
describe('App Integration', () => {
    it('renders the main heading', () => {
        render(<App />);
        expect(screen.getByRole('heading', { level: 1, name: /デジタル資産相続ツール/i })).toBeInTheDocument();
    });

    it('renders accordion sections', () => {
        render(<App />);
        expect(screen.getByRole('heading', { level: 3, name: /デジタル資産相続ツール/i })).toBeInTheDocument();
        expect(screen.getByRole('heading', { level: 3, name: /使い方/i })).toBeInTheDocument();
    });
    ...
    it('interacts with accordion', async () => {
        ...
        expect(screen.getByText(/「データをスキャンする」のボタンから追加できます/i)).toBeInTheDocument();
